...
import { BloombergRaw } from './shared';
import { PublicationRaw } from '../shared';

@Injectable()
export class BloombergService {
  private readonly logger = new Logger(BloombergService.name);
  ...
  async getSub(canonical: string, optIn: any) {
    ...
    const publicationRaw = PublicationRaw.bloombergFactory(data);
  ...
  async fetchSub(canonical: string) {
    return new Promise((resolve, reject) => {
      ...
      const params = {
        url: canonical,
        api_key: token,
        wait_for: '.body-content.fence-body p.paywall',
        extract_rules: JSON.stringify({
          contents: {
            selector: '.body-content.fence-body p',
            type: 'list',
          },
          author: {
            selector: '.author__619cf27c a',
          },
          published: {
            selector: `meta[name="iso-8601-publish-date"]`,
            output: '@content',
          },
          canonical: {
