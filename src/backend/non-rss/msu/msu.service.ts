...
import { MSUMainRaw, MSUSubRaw } from './shared';
...
export class MSUService {
  ...
  async fetchMain() {
    return new Promise((resolve, reject) => {
      ...
      const msuHost = this.configService.get<string>('MSU_HOST');
      const msuMain = this.configService.get<string>('MSU_MAIN');
      const params = {
        url: `https://${msuHost}/${msuMain}`,
        api_key: token,
        render_js: false,
        extract_rules: {
          articles: {
            selector:
              '.entry-content .js-wpv-loop-wrapper .tb-grid .tb-grid-column',
            type: 'list',
            output: {
              title: {
                selector: '.tb-heading',
  ...
  async getSubs(data: MSUMainRaw, optIn: any) {
    optIn.totals = data.articles?.length || 0;
    optIn.countCreated = 0;
    optIn.countUpdated = 0;

    for (let i = 0; i < data.articles?.length; i++) {
      const { canonical: canonicalOrigin, title } = data.articles[i];

      const opt = { error: null };
      const msuSubRaw = await this.getSub(canonicalOrigin, opt);
      if (opt.error) {
      ...
      const { author, contents, canonical } = msuSubRaw;
      ...
      const createStreamEntries = CreateStreamEntries.msuEntryRawFactory({
        entryId,
        canonical,
        canonicalOrigin,
        title,
  ...
  async getSub(canonical: string, optIn: any): Promise<MSUSubRaw> {
